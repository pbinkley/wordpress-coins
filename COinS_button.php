...
function pab_COinS_handler(URL) {
   ...
   formWin.focus();
      ...
      +'	addField("epage", mainform.epage.value);'
			+' opener.pab_insert_COinS(contextObject);'
			+' self.close();'
      +'}'
			+'</scr' + 'ipt>'
      +'</head>'
...
{
		edInsertContent(edCanvas, '<span class="Z3988" title="ctx_ver=Z39.88-2004&amp;rft_val_fmt=info:ofi/fmt:kev:mtx:journal&amp;' + contextObject + '">'
